...
describe('Render Picture React Base', () => {
  ...
  it('should contain the Back and Forward button & PhotoSlider Components', () => {
    const wrapper = mount(<PhotoCarousel currentPhoto={mockPhotos[0]} photos={mockPhotos}/>);
    expect(wrapper.children().childAt(1).childAt(0).childAt(0).name()).toBe("BackButton");
    expect(wrapper.children().childAt(1).childAt(2).childAt(0).name()).toBe("ForwardButton");
    expect(wrapper.children().childAt(1).childAt(1).childAt(1).name()).toBe("PhotoSlider");
  });
  it('Exit/Back/Forward Buttons to be svg tags', () => {
    ...
    expect(wrapper.children().childAt(1).childAt(0).childAt(0).html()).toEqual(expect.stringContaining('svg'));
    expect(wrapper.children().childAt(1).childAt(2).childAt(0).html()).toEqual(expect.stringContaining('svg'));
