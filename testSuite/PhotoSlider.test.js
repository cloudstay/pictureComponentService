...
describe('Render Picture React Base', () => {
  ...
  it('should toggle button Show Photo List and Hide Photo List', () => {
    ...
    wrapper.setProps({showPhotoSlider : true});
    wrapper.update();
    expect(wrapper.children().childAt(0).childAt(1).children().children().childAt(0).text()).toBe('Hide photo list');
    wrapper.setProps({showPhotoSlider : false});
    wrapper.update();
    expect(wrapper.children().childAt(0).childAt(1).children().children().childAt(0).text()).toBe('Show photo list');
  });
});